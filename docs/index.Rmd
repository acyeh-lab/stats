...
**There is NOT a simple formula for determining sample size for every research situation.** The reality it that there are many research situations that are so complex that they almost defy rational power analysis. In most cases, power analysis involves a number of simplifying assumptions, in order to make the problem tractable, and running the power analysis numerous times with different variations to cover all of the contingencies.

### Implementation
We can compute a power analysis using functions from the `pwr` package. The `tidyr`, `dplyr`, and `kableExtra` packages will be used to help generate / manipulate / display data  and `ggplot2` will be used for plotting data. Make sure to install the packages first, e.g. `install.packages("pwr")`
...
## Part 2: Power Analyses - Extension to Other Outcome Measurements
...
### Effect sizes
Power calculations to determine sample size requires an input for effect size. Different test outcomes calculate effect sizes differently - common situations are shown below, along with the generally accepted "small", "medium", and "large" effect sizes.
...
df<-data.frame(rbind(t_test,prop_test,chi_sq,ANOVA,pearson,linear_reg))
colnames(df) <- c("Effect Size", "Small", "Medium","Large")
row.names(df) <- c("T-test", "Test of proportion","Chi-squared test (X^2)","Analysis of variance (ANOVA)", "Pearson's correlation", "Linear regression")
...
### Modeling in R
To perform power calculations in R, one can use the `pwr` package per above.  Common functions included in the package for power calculations are shown below:
```{r power analyses commands, echo=FALSE, results='markup'}
t_test <- c("pwr.t2n.test(n1 = , n2= , d = , sig.level =, power = )")
prop_test <- c("pwr.2p2n.test(h = , n1 = , n2 = , sig.level = , power = )")
chi_sq <- c("pwr.chisq.test(w =, N = , df = , sig.level =, power = )")
ANOVA <- c("pwr.anova.test(k = , n = , f = , sig.level = , power = )")
pearson <- c("pwr.r.test(n = , r = , sig.level = , power = )")
linear_reg <- c("pwr.f2.test(u =, v = , f2 = , sig.level = , power = )")
df<-data.frame(rbind(t_test,prop_test,chi_sq,ANOVA,pearson,linear_reg))
colnames(df) <- c("R function using 'pwr' package")
row.names(df) <- c("T-test", "Test of proportion","Chi-squared test (X^2)","Analysis of variance (ANOVA)", "Pearson's correlation", "Linear regression")
...
#### Finding the Power
If one looks at the power calculation for linear regression, the effect size is noted as "f^2", where f^2 = R^2 / (1-R^2).  We also see two other variables we haven't encountered: "u" and "v".  "u" is the number of dependent variables minus 1 (a.k.a. numerator degrees of freedom), where "v" is the sample size minus dependent variables (a.k.a. denominator degrees of freedom).
...
```{r, echo=TRUE, results='markup'}
# example: k=5 predictors, n=Unknown observations, R^2 = .15, sig.level = .05 (default)
pwr.f2.test(u = 4,
            v = NULL,
            f2 = .15/.85,
            sig.level = 0.05,
...
```{r, echo=TRUE, results='markup'}
# example: k=Unknown, n=100 observations, R^2 = .15, sig.level = .05 (default)
pwr.f2.test(u = NULL,
            v = 100,
            f2 = .15/.85,
            sig.level = 0.05,
...
### Chi-squared test
For the chi-square test, the effect size index w is calculated by dividing the chi-square value by the number of scores and taking the square root.
