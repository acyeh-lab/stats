...
library(WebPower) # Advanced power analysis package
...
### Modeling in R
Common functions included in the package for power calculations are shown below:
...
df<-data.frame(rbind(t_test,prop_test,chi_sq,ANOVA,pearson,linear_reg,logit_reg))
colnames(df) <- c("R package", "R function using 'pwr' package")
row.names(df) <- c("T-test", "Test of proportion","Chi-squared test (X^2)","Analysis of variance (ANOVA)", "Pearson's correlation", "Linear regression", "Logistic regression")
...
### Logistic Regression 
Unlike a linear regression where the outcome is continuous (e.g. diastolic blood pressure change), a logistic regression is used to predict a binary outcome (e.g. improvement or no improvement), based on prior observations of a data set.

Unlike the other tests mentioned before, there is not one "effect size" statistic for logistic regression, but power calculation can still be performed if we can estimate the following two statistics instead:
